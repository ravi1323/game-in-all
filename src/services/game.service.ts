...
export class Game {
    ...
    constructor({
        ...
        if (mProduction && (!key || key === '' || !cert || cert === '')) throw new Error(`'{key}' & '{cert}' is required on production.`);
        else {
            this.httpServer = mProduction ? new HttpServer({ production: mProduction, port: httpPort, key, cert}) : new HttpServer({ production: mProduction, port: httpPort});
            const socketServer = new SocketIO({ httpServer: this.httpServer });
            this.IO = socketServer.getIO();

            this.IO.on(CONSTANTS.SOCKET.EVENTS.CORE.CONNECT, async (socket: Socket) => {
                let handlers = {}

                handlers[`${CONSTANTS.SOCKET.EVENTS.CUSTOM.SIGNUP}`] = async (data, acknowledgement, socket, eventName) => this.signup(data, acknowledgement, socket, eventName);
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (!isValid.valid) {
                    ...
                    socket.emit(eventName, makeResponse({
                        msg: 'signup done!'
                    }))
                }
            } catch(e) {
